...
# -*- coding: utf-8 -*-
"""
PD 통합 소프트웨어 v2.1 - 성능 최적화 버전
GUI 응답성과 NDI 처리 성능이 대폭 개선된 버전
"""
...
logger.info("="*80)
logger.info("PD 통합 소프트웨어 v2.1 (최적화 버전) 시작")
logger.info("="*80)
...
# 성능 최적화 설정
os.environ['QT_ENABLE_HIGHDPI_SCALING'] = '0'  # High DPI 스케일링 비활성화
...
def setup_application_optimization():
    """애플리케이션 레벨 최적화 설정"""
    # Qt 애플리케이션 속성 설정 (PyQt6)
    ...
    if sys.platform == "win32":
        try:
            ...
            # 프로세스를 HIGH_PRIORITY_CLASS로 설정
            handle = ctypes.windll.kernel32.GetCurrentProcess()
            ctypes.windll.kernel32.SetPriorityClass(handle, 0x80)  # HIGH_PRIORITY_CLASS
            logger.info("프로세스 우선순위를 HIGH로 설정")
        except Exception as e:
            logger.warning(f"프로세스 우선순위 설정 실패: {e}")
...
def main():
    ...
    # 애플리케이션 최적화 설정
    setup_application_optimization()
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"일부 Qt 속성이 없습니다: {e}")
        
        # 스타일 설정
        app.setStyle('Fusion')
        app.setApplicationName("PD 통합 소프트웨어 v2.1 (최적화)")
        ...
        # 메인 윈도우 생성 - 최적화 및 오류 수정 버전 사용
        logger.info("메인 윈도우 생성 시작")
        try:
            # 최적화 및 오류 수정 메인 윈도우 임포트 시도
            try:
                from pd_app.ui.main_window_optimized_fixed import MainWindowOptimizedFixed as MainWindow
                logger.info("최적화 및 오류 수정 메인 윈도우 사용")
            except ImportError:
                try:
                    from pd_app.ui.main_window_optimized import MainWindowOptimized as MainWindow
                    logger.info("최적화된 메인 윈도우 사용")
                except ImportError:
                    from pd_app.ui import MainWindow
                    logger.info("기본 메인 윈도우 사용")
