# pd_app/ui/main_window_optimized_fixed.py
"""
통합 메인 윈도우 - 성능 최적화 및 오류 수정 버전
GUI 응답성 개선 및 안정성 강화
"""
...
class MainWindowOptimizedFixed(QMainWindow):
    """성능 최적화 및 안정성 강화된 통합 메인 윈도우"""
    ...
    def init_ui(self):
        ...
        # 탭 변경 시 불필요한 업데이트 방지
        self.tab_widget.currentChanged.connect(self.on_tab_changed_safe)
    ...
    def on_tab_changed_safe(self, index):
        ...
        try:
            ...
            # 사용자에게는 치명적이지 않은 오류이므로 메시지 박스 표시하지 않음
            # 대신 상태바에 오류 표시
    ...
    def on_tab_changed(self, index):
        """탭 변경 시 지연 로딩"""
        # 이미 초기화된 탭이면 스킵
        if index > 0 and self.tab_initialized.get(index, False):
            return
            
        try:
            # NDI 탭
            if index == 1 and self.ndi_widget is None:
                logger.info("NDI 위젯 초기화 시작")
                ...
                self.tab_initialized[1] = True
                logger.info("NDI 위젯 초기화 완료")
                    
            # vMix Tally 탭
            elif index == 2 and self.tally_widget is None:
                logger.info("Tally 위젯 초기화 시작")
                ...
                if old_widget:
                    old_widget.deleteLater()
                self.tab_initialized[2] = True
                logger.info("Tally 위젯 초기화 완료")
                    
            # SRT 스트리밍 탭
            elif index == 3 and self.srt_widget is None:
                logger.info("SRT 위젯 초기화 시작")
                ...
                if old_widget:
                    old_widget.deleteLater()
                self.tab_initialized[3] = True
                logger.info("SRT 위젯 초기화 완료")
                
            # 설정 탭
            elif index == 4 and self.settings_widget is None:
                logger.info("설정 위젯 초기화 시작")
                # 임시 설정 위젯
                settings_widget = QWidget()
                settings_layout = QVBoxLayout(settings_widget)
                settings_layout.addWidget(QLabel("설정 페이지 (준비 중)"))
                settings_layout.addStretch()
                
                self.settings_widget = settings_widget
                old_widget = self.tab_widget.widget(4)
                self.tab_widget.removeTab(4)
                self.tab_widget.insertTab(4, self.settings_widget, "설정")
                self.tab_widget.setCurrentIndex(4)
                if old_widget:
                    old_widget.deleteLater()
                self.tab_initialized[4] = True
                logger.info("설정 위젯 초기화 완료")
                
        except Exception as e:
            logger.error(f"위젯 초기화 실패 (탭 {index}): {e}", exc_info=True)
            # 실패한 탭은 다시 시도할 수 있도록 상태 리셋
            self.tab_initialized[index] = False
            raise
    ...
    def on_auth_state_changed(self, is_authenticated):
        ...
        try:
            ...
            logger.error(f"인증 상태 변경 처리 오류: {e}")
    ...
    def on_ws_connection_changed(self, is_connected):
        ...
        try:
            ...
            logger.error(f"WebSocket 상태 변경 처리 오류: {e}")
    ...
    def on_logout_success(self):
        ...
        try:
            ...
            logger.error(f"로그아웃 성공 처리 오류: {e}")
