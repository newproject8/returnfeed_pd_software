...
from PyQt6.QtGui import QAction, QKeySequence, QShortcut, QImage, QIcon
import logging
import psutil
import time
import re
...
class ClassicMainWindow(QMainWindow):
    ...
    def _init_ui(self):
        """사용자 인터페이스 초기화"""
        self.setWindowTitle("리턴피드 -전 스태프 초고속 PGM,탈리신호 공유 (v0.9)")
        ...
        paths_to_try = [
            ...
            os.path.join("C:", "coding", "returnfeed_tally_fresh", "returnfeed_unified", "resource", logo_filename),
            os.path.join("/mnt/c/coding/returnfeed_tally_fresh/returnfeed_unified/resource", logo_filename),
        ]
        ...
        for path in paths_to_try:
            if path and os.path.exists(path):
                ...
                if not icon.isNull():
                    ...
                    logger.info(f"Window icon loaded successfully from: {path}")
                    logo_loaded = True
                    break
        
        if not logo_loaded:
            logger.warning(f"Window icon not found. Tried paths: {[p for p in paths_to_try if p]}")
        ...
        main_layout.setSpacing(1)  # 상단행과 동일한 느낌을 위한 최소 간격
